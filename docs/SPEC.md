...
| inventory_deadlines_feed_token | TEXT | Nullable (migration v144) - secret token of this member's read-only inventory deadlines ICS feed. Partial UNIQUE index WHERE NOT NULL. See [Inventory](#inventory-inventory) |
| cycle_feed_token | TEXT | Nullable (migration v180) - secret token of this member's predicted-cycle ICS feed. Partial UNIQUE index WHERE NOT NULL. See Health → Cycle |
| schedule_feed_token | TEXT | Nullable (migration v183) - secret token of this member's schedule ICS feed. Partial UNIQUE index WHERE NOT NULL. See [Schedule](#schedule-migration-165-786) |
| schedule_reminder_offset_minutes | INTEGER | Nullable (migration v184) - lead time of shift-start reminders in minutes; `NULL` = off (the default) |
| schedule_weekly_hours | INTEGER | Nullable (migration v185) - personal weekly-hours target for the overtime flag; `NULL` falls back to 40 |
...
or clearing a match needs `mayWriteModule(req, 'pantry')`. Hiding the control in the client is a
rendering choice, not the boundary.
...
**Private network targets (v2.64.1 addendum, #1053).** Since GHSA-9jh6-phj9-m6qr the SSRF hook in
`safeRequest()` also checks an IP literal on the first hop, so a `base_url` like `http://192.168.x.x`
that used to slip past it now needs `RECIPE_PROVIDER_ALLOW_PRIVATE_NETWORK=true` (default `false`).
`server/services/recipe-providers/private-network.js` is the one place for that opt-in and for the
answer an admin gets without it: the account form refuses a local name or private literal before any
network call and names the switch, and a hostname that resolves into a private network gets the same
hint on the connection test, the account card and after a failed sync.
...
**A later target choice withdraws a queued move (#593).** A field edit writes the move target
through `COALESCE`, so it cannot swallow a move that is still waiting - and the same `COALESCE`
kept a queued move alive against the user: choosing the calendar the event already sits in derived
no move and therefore wrote nothing, so a move whose attempt had failed still ran on the next sync.
`markEventOutbound()` now withdraws the queued move explicitly when the target chosen *in the
request* points back at the current calendar, and replaces it when it points at a third one. The
rule above still holds: an edit that leaves the target alone keeps the queued move, and a stored
target that differs from the actual calendar is never read as a wish to move. Withdrawing is a
local booking and needs no write access at the provider, so it also goes through while the sync is
read-only or its account is gone; queueing new outbound work still does not.

**One outbound pass per provider (#1151).** The immediate attempt behind every write route and the
scheduler's sync run keep the same bookkeeping (tombstones, `outbound_dirty`, `outbound_move_to`)
across network calls, so two overlapping passes could clear each other's markers between two
requests. `server/utils/sync-lock.js` serialises them per provider: the immediate attempt, the
scheduled sync and a second scheduled tick wait for each other, and a burst of writes during a slow
...
several people can share one entry. Only household members can be chosen (#1207): a newly named
staff member or split-expense guest is refused with 400, while one the entry already names stays
valid when it is saved again.
...
Only `task`, `event`, `subscription`, `inventory_item` and `inventory_tracked_date` can be set and
deleted through `POST`/`PUT`/`DELETE /api/v1/reminders` (`SETTABLE_ENTITY_TYPES` in
...
qualifying pattern-day row and at most one override anchor per day), `cycle_reminder_anchors`
(migration v177, UNIQUE on `(user_id, anchor_date, kind)`) and `waste_reminder_entries` (migration v203).
None of them is something a caller could construct even if the type were settable.
...
**Target validation (v2.64.1, GHSA-f4w5-ggcc-7m5c).** Webhook, Gotify and ntfy deliver through
`server/services/notification-providers/guarded-fetch.js`, which uses the same node-native client
(`safeRequest()` in `server/utils/http.js`) and anti-rebinding lookup (`server/utils/ssrf.js`) as ICS
subscriptions and the recipe mirrors - the address actually connected to is checked, on every redirect
hop. Before that the three providers called the bare global `fetch()` on the configured URL. Saving a
channel refuses a private or local hostname or IP literal (`server/services/notification-channels.js`),
naming the switch in the message. `NOTIFICATION_ALLOW_PRIVATE_NETWORK=true` lifts the guard for a
Gotify/ntfy server in the same Docker network or LAN; the default is off, like the ICS feeds' switch,
and it is a deployment switch rather than an app setting because the line between app admin and host
operator runs exactly here.
...
Tokens can optionally be **scoped** to individual modules and access levels — a least-privilege allow-list that matters most for tokens handed to an off-device AI/MCP client. Each scope is `<module>:read` or `<module>:write` (write implies read); modules cover `tasks`, `shopping`, `meals`, `pantry`, `inventory`, `calendar`, `notes`, `contacts`, `schedule`, `budget`, `documents`, `health`, `rewards`, `housekeeping`, `waste`, `weather`, `family`, `dashboard`, `search` (`SCOPE_MODULES` in `server/scopes.js`, plus the modules third-party extensions register). **A scoped token reaches no `/api/v1/auth/*` route at all (v2.57.1, GHSA-xcv5-6w6x-x5q2):** the auth router is mounted ahead of the global scope, guest and module gates so login, setup and the OIDC handshake stay reachable without a session, and it therefore re-checks at its own entry (`server/auth.js`) - otherwise a token limited to one read scope, acting as an admin subject, could mint an unscoped token or a new admin. Unscoped tokens and sessions are unaffected. A `NULL` scopes value means no scoping — full role-based access (the default, and the state of every token created before this feature). A scoped token can only reach modules on its allow-list; every other `/api/v1` path is denied. Enforcement is shared across the REST API and MCP: the MCP core tools are checked in-process, `tools/list` hides tools the token cannot use, and the OpenAPI bridge inherits the same limits because it loops back through the REST layer with the same token. Scopes narrow, they never grant: the module permissions of the user behind the token apply on top of them, on both surfaces (#823). Endpoints that aggregate several modules - global search and the dashboard - pass the path check on their own scope (`search`, `dashboard`) and then drop every part whose module the token cannot read, the same way they drop a module the member has no access to: `search:read` opens the search, not the modules behind it.
...
"origin-to-module permission filtering" `server/services/reminder-origins.js#ORIGIN_MODULE` already enforces
...
current array format and, for backward compatibility with pre-migration-178 clients, a comma
...
| note | TEXT | optional |
| reminder_offset_minutes | INTEGER | optional, own per-extra shift-start reminder lead time — independent of the member's own `users.schedule_reminder_offset_minutes` below |
| created_by | INTEGER | FK → Users (SET NULL) |
| created_at | TEXT | ISO 8601 |
...
187, the seventh widening of `reminders.entity_type`) and their **own** configurable offset
(`reminder_offset_minutes` above) rather than inheriting the member's general shift-start setting — on-call
...
anchor slot, which is keyed by `date_key` and `pattern_day_id` (migration 188 allows several
pattern-day rows on one day).
...
migration 183, partial UNIQUE index) identifies the subscriber, and the feed content itself is
...
- **Shift-start reminders:** an opt-in push notification before an upcoming shift begins.
  ...
  reminder origin - `schedule_reminder_entries` (migration v184) is a small anchor table, one row per
  qualifying slot (UNIQUE on `(user_id, date_key, COALESCE(pattern_day_id, 0))`, so a timetable day
  with several pattern-day rows gets one anchor each), that `server/services/schedule-reminders.js`'s periodic sync
...
- **Weekly-hours target (`schedule_weekly_hours`, migration v185):** the personal full-time figure
...
- **Overtime tracking toggle (`schedule_overtime_enabled`, migration v208; a nullable INTEGER in
  the column, a plain boolean without a null state in the API):** whether the overtime flag runs at all for this person, independent of the weekly-hours
...
**The path is matched case-insensitively (v2.66.1, GHSA-cvwj-hx37-3r7m).** Express routes without
regard to letter case, so `/api/v1/Notes` reaches the notes router. `moduleForPath()` in
`server/scopes.js` compared the prefix letter for letter, found no module for the capitalised spelling
and returned `null`, which the module deny-list in `server/index.js` lets through: a member set to
`none` could read the module that way, and one with `read` could write to it. It now folds the path to
lower case before matching (every prefix is lower case). Scoped tokens were never let through, but
were refused for a capitalised path to a module they may use; that works now as well.
...
allowed, plus "inherit" for a member override).
...
**Weather and Clock carry the same header.** They were the two widgets that opened straight into
their content; both now start with the seal and title their neighbours carry, while the temperature
and the clock face stay as prominent as before. The metrics row is left without one on purpose:
each of its tiles already names a different module with its own seal and label, and one title over
several modules would misname it.
...
- Today's meals: meals for the current day. A meal whose recipe has a picture shows it as a thumbnail in its title row (the recipe's own upload wins over the provider thumbnail); a meal without one gets no placeholder and keeps its title width (#1059)
- Pinboard preview: pinned notes first, then the most recently changed, with Markdown formatting rendered. The payload supplies five and the tile decides how many appear (`listRowCap`, same as birthdays) - three rows for a one-row tile, five for a taller one. It used to cut at three on the server for every size, which made three the ceiling for the 1×2 default the tile ships at: a household with five pinned notes saw three, with the metric tile beside it saying "5 pinned" (#928). The list is a **preview and not a filter** - past what the tile holds, `/notes` is where the rest lives, and the pin ordering plus the `updated_at` touch a pin write causes means a freshly pinned note is always at the front of it. **Category option (#1044):** the widget's options can limit the tile to notes carrying every chosen category. The browser sends them as a repeatable `?notes_category=` (positive ids, deduplicated, at most 50, AND), and the route applies them before the row limit and to `pinnedNotesCount` alike - the #814 rule. A category only matches when it is a household one or belongs to the caller, so a guessed id of another member's personal category reveals nothing, and `GET /preferences` drops ids from the stored options that the caller can no longer see
...
- Waste (#1063): the next pickup per active waste type, soonest first (`GET /waste/occurrences/next`), with the "needs a refresh" flag from `GET /waste/sources`. Like Schedule and Cycle its own slice: `/dashboard` never carries it, the tile fetches it client-side and only while it is visible. The widget option `types` limits it to chosen types; no selection means all. Default-hidden (`DEFAULT_HIDDEN_WIDGETS` in `public/utils/dashboard-widgets.js`), like the module itself
...
The newer modules (Rewards, Health, Housekeeping, Schedule, Waste) start **hidden** by default — they are specialised and not active in every household, so they are offered as opt-ins in **Customize** rather than adding empty tiles to a fresh dashboard. Existing saved layouts are untouched.
...
- **Recipe integration:** Select a saved recipe from the meal modal to auto-fill title, notes, URL, and ingredients. Scale ingredient quantities by a numeric factor. Scaling reads and writes the number in the household's region - its decimal and group separators and its digit system, through `public/utils/digits.js`, which the server shares - so "1,000 g" under en-US is scaled from 1000 and a Persian "۲ x ۵۰۰ g" stays in Persian digits. A leading fraction ("1 1/2 cups") is read as well; a line whose number cannot be read unambiguously ("1/2,5 cup") is left exactly as written. When a meal plan goes to the shopping list, the server adds up the same ingredient across meals and accepts a quantity in any digit system. Save the current meal as a new recipe with one click.
...
- **Household names for the four slots (#1058, v2.66.0):** set in Settings → Modules → Kitchen next to the visibility switches and stored in `sync_config` as `meal_type_names`, a JSON object keyed by slot (read through `parseMealTypeNames()` in `server/routes/preferences.js`: only the four slot keys, trimmed; a name longer than 40 characters or a non-string value is rejected with 400, an empty name falls back to the translated built-in word, `null` clears all). A name is not a translation: it shows exactly as typed in every language, because no locale file can settle what a household calls its evening meal. The slot **keys** stay unchanged, so recipe suitability, the Mealie/Tandoor mapping and the planner rows are unaffected and nothing migrates.
- **Recipe pictures on planned meals (#1059, v2.66.0):** a meal planned from a recipe shows the recipe's own image (`recipes.image_data`, migration v192) or, failing that, the provider thumbnail (`recipe_has_own_image` / `recipe_has_image` on the meal); the own image wins where both exist. A card without a picture keeps its previous layout, with no placeholder slot.
...
- **"Unassigned" on the person axis (#1064):** the person list gains a row for the events and tasks nobody is assigned to, stored as the value `none` in the same `yuvomi:calendar:people` list (a string, so it cannot collide with a user id). On its own it shows only the unassigned entries; together with people it adds them. A person filter saved before this row existed does not contain it, so unassigned events stay out under it, as they did (#987).
- **Calendar and subscription filter (#1064):** the filter sheet lists every connected calendar and ICS subscription with events in the loaded range, each with its colour and a switch; hiding one removes its events from all four views and the agenda. It is a filter, not the colour legend the person filter above deliberately avoids: an event's colour still resolves through `resolveEventColor()`. The hidden set is stored on the device **per account** (`yuvomi:calendar:sources-hidden:<userId>`) together with each source's name and colour, so a hidden source stays listed and can be switched back on even when none of its events is on screen, and a private subscription's name does not show to the next account on a shared browser. A new event created for a hidden calendar stays out before it has synced, and an event with a pending move counts under its destination calendar.
- **Month titles on a phone (v2.65.0):** below 640px a month cell reduces every entry to a dot by default. An "Event titles" switch under Display in the filter sheet keeps the chips instead, as compact single-line rows (`month-view--titles`; the dot rules in `public/styles/calendar.css` are scoped to `:not(.month-view--titles)`). Colour, contrast and the tap target of the cell stay as they are. Stored per device, not per household (`yuvomi:calendar:month-titles` in `localStorage`, default off), because whether a title fits is a question of the screen, and the same person reads the same calendar on a monitor in the evening.
...
- **Default assignee per sync target (migration v79):** each synced calendar (Google/CalDAV) and each ICS subscription can be given an optional default assignee in Settings → Sync; newly imported events of that target are auto-assigned to that person (new events only — see [External Calendars](#external-calendars)). The per-calendar picker appears once the calendar has completed its first sync. **Since #1060 it is read in the other direction too:** a new event assigned to exactly one person gets as its target the one Google or CalDAV calendar that names that person (`defaultAssigneeUserId` on `GET /api/v1/calendar/sync-targets`, rule in `assigneeSyncTarget()` in `public/utils/sync-target.js`). It applies only when creating, and only until the target is chosen by hand; without a match the member's own default target applies as before (#620). Two assignees pick nothing - the dialog says so and names the target used instead, as soon as a calendar names at least one of them (`severalAssignees`, #1332); with none of them named there is nothing the assignment would have picked, and the dialog stays silent as it does for one person without a calendar. Two calendars naming the same person pick nothing either - the dialog says so instead of guessing, and the settings do not refuse that setup, because imports like "Emma school" and "Emma sport" rely on it. Apple has no target format and Outlook no default assignee, so neither is chosen this way; existing events never move on their own
...
- **Event location:** Event popup and dashboard display the location field with RFC 5545 backslash-escape normalization (`\n`, `\,`, `\;`, `\\`) via `fmtLocation()` in `public/utils/html.js`. **Open in Maps (#1110):** the event detail carries an explicit action whenever the event has a location; it opens an OpenStreetMap search for that text in a new tab, the same vendor-neutral search Contacts uses for an address. It is an action rather than a link on the location itself, because the field is free text ("Zoom" or "Room 3B" is not an address). The URL is built on the device and used only when tapped - no geocoding, nothing looked up in advance - and an address imported over CalDAV with escaped line breaks is searched as one line.
...
- **Categories (#1044):** the board manages the catalog described under [Note Categories](#note-categories-migration-v176) through the shared category manager - every member their own personal categories, the household catalog whoever has admin access or the `notes_manage_household_categories` capability. The note editor carries a category picker, and household categories stay assignable for members who cannot manage them. A chip row filters the board to notes carrying **every** selected category (AND, `noteMatchesCategories()` in `public/utils/note-category-filter.js`); a selected category that has since been deleted is dropped from the selection on the next render. Badges on a card stay on one line; a `+N` control reveals the remaining categories on hover, focus or tap and announces them.
...
- **Folder upload (v2.65.0):** a whole directory can be picked (`webkitdirectory`; not offered on iOS/iPadOS, `supportsDirectoryUpload()`). It becomes a new folder below the chosen destination, and its descendant folders and supported files are recreated and uploaded one after another. Before anything is written the dialog previews the planned tree, the conflicts and the rejected files: an existing folder is merged or duplicated under a timestamped name, a conflicting file skipped or uploaded under a timestamped name. The plan follows the server's size, MIME-type, path and depth limits (`MAX_FOLDER_DEPTH`, five levels); a rejected parent blocks only its own descendants, not its siblings. When the server's rate limit answers, the upload waits for the advertised retry time and resumes; cancelling stops the queue and keeps what was already created. Planning and outcome rules live in `public/utils/folder-upload.js`, outside the modal code, so they run in Node tests
- **Folder browser:** documents can be organized into custom folders; a sidebar lists all folders plus "Alle Dokumente" and "Kein Ordner". The first row is the neutral state of a filter, not an overview of folders, and it is named for what it selects: it carries a document count, and calling it "Alle Ordner" next to that count read as a number of folders (v2.8.2, #757). **Folders nest (migration v164, #785).** They were flat until then, and the request was to hang them under the categories - which does not work, because the two axes are independent: a folder "Apartment" holds documents categorised `home`, `insurance` and `legal` at the same time, so under a category it would stand three times or carry a membership that does not exist. The hierarchy therefore sits where it belongs - `family_document_folders.parent_id`, capped at **five** levels (where the indentation stops leaving room for the name on a phone) - and the category stays a cross-cutting label on the document. `name` lost its global UNIQUE in the same migration and is now unique per sibling row (`COALESCE(parent_id, 0), name`): "Invoices" under "Car" and under "Apartment" are two folders. `COALESCE` and not a plain `UNIQUE(parent_id, name)`, because SQLite treats each NULL in a UNIQUE as distinct and would allow any number of same-named root folders. Selecting a folder shows the documents of its whole subtree, and the count beside it answers the same question - the rule for that lives once, in `public/utils/folder-tree.js`, and is used by both the sidebar and the route. Custom folders can be created, renamed, and deleted (via a per-folder overflow menu); deleting a folder keeps its documents (their folder link is cleared) but takes its whole subtree with it (`ON DELETE CASCADE` on `parent_id`, v164) - the confirmation names how many subfolders go along, because the sidebar only shows the collapsed root. **Deleting a folder is a two-step choice (#989):** the dialog previews the exact number of folders and documents in the subtree, and offers two outcomes - keep the documents (unfiled, reachable through the no-folder view) or delete the confirmed subtree together with its documents. The destructive option is offered only when the caller may delete every affected document, and the subtree is selected through `documentVisibleSql()` and refused as soon as one row in it is invisible to the caller, with no admin bypass (DECISIONS.md entry 1). The server binds the confirmation to the previewed identities, so a folder or document that moved in between makes the request fail rather than delete something the person never saw; while external storage is being changed the affected rows are locked against concurrent moves, and if a storage deletion fails the folder structure is preserved. The destructive variant runs behind the shared Undo toast (v2.65.0, `scheduleFolderDeleteWithUndo()` in `public/utils/document-folder-delete.js`): the subtree and its visible documents disappear at once and the request is sent only when the window closes; leaving the page flushes it, and a delayed request that fails brings the folder back and reports the error. New uploads are pre-assigned to the currently selected folder. Six modules file their receipts in a system folder of their own (Budget, Tasks, Shared expenses, Inventory, Housekeeping, calendar attachments); since v157 that folder is identified by `family_document_folders.module_key`, not by its translated name, so two members with different languages file into the SAME folder and renaming either the folder or its translation no longer creates a second one. The name is a label and may be changed freely. The housekeeping folder is auto-created when the first housekeeping worker is added
...
- **API:** `GET /api/v1/housekeeping/visits/:id` returns a single work session with worker name, task list, and linked document; `POST /api/v1/housekeeping/visits/:id/unpay` (admin) takes a payment back
...
Optional module for a household's waste pickups (#1063). **Disabled by default:** migration v198 adds
`waste` to `disabled_modules` with the same merge-not-replace pattern as Inventory and Schedule, so a
household opts in under Settings → Modules. Module key, permission entry and token scope are all
`waste` (`server/permissions.js`, `server/scopes.js`); the API lives under `/api/v1/waste`
(`server/routes/waste/`, contracts in `server/openapi/paths/waste.js`). Data model and occurrence
resolution: see [Waste Types](#waste-types-migration-v197-1063) and the sections after it.

- **Page (`public/pages/waste.js`):** waste types, their weekly, fixed-day-of-month or ordinal-weekday
  schedules, moving or skipping a single calculated occurrence, one-off pickups, the upcoming list
  (`UPCOMING_WINDOW_DAYS`, 90 days) and the ICS file and URL sources. The page leads with a labelled
...
- **Calendar layer:** device-local and off by default (`yuvomi:calendar:layer:waste`), with the
  ...
  Waste never writes `calendar_events`.
- **Reminders and feed:** each member opts into pickup reminders per type (see Waste Reminder
  Settings); the personal ICS feed is managed on Settings → Personal → Feed subscriptions
  (`public/settings/pages/personal-feeds.js`), next to the other personal feeds.
- **Search:** a waste type found through the global search opens `/waste?type=<id>`.
...
- **Household size (v2.0.2):** `GET /api/v1/auth/me` and `POST /api/v1/auth/login` return `householdSize`, the number of household members - without housekeeping staff and split-expense guests, the same predicate every list of members uses (#1207). A household of one person and a cleaner is a household of one. The client holds it in `utils/household.js` and mirrors it to a root class, so the interface can leave out what has only one sensible answer in a household of one. It is a **presentation** signal: no stored value depends on it. **Protective controls follow a second value:** the same responses carry `othersCanRead`, the modules among `calendar`, `documents` and `tasks` that any other account can read according to its resolved permissions - housekeeping staff included, split-expense guests not (they reach no route outside shared expenses). Task visibility and lock, document visibility and shares, and event visibility stay in the interface while their module is named (`hidesPrivacyControls(module)`), because without them a new entry stays readable for exactly those accounts.
...
- **Configurable reminder:** customizable reminder offset per birthday with preset options (none, at time, 15 min, 1 h, 1 d, 2 d, 1 w, 2 w) and a fully custom interval (amount + unit). Reminder time calculated from offset; auto-dismissed when the birthday passes. **A dismissed reminder stays dismissed (#1163):** the sync that keeps a birthday's reminder in step reuses the row for the same time, dismissed or not, and replaces it only when the time itself changes - a different lead time, a changed date, or next year's birthday. It used to look for an undismissed row only, delete the dismissed one and insert the same reminder again, so the reminder was back within a minute and could be pushed a second time
...
- **Birthday reminders** auto-synced from the Birthdays module (configurable offset per birthday, counted back from 12:00 on the day in the household time zone; the editor starts a new birthday at 1 day before, and one without a stored offset is reminded on the day itself)
- Dismissing a reminder marks it `dismissed = 1`; dismissed reminders are not shown again
- **A module switched off for the household sends no reminders (#1279):** neither the push/channel delivery nor `GET /reminders/pending` hands out a row whose origin belongs to a module in `disabled_modules` (`entity_type` → module via `server/services/reminder-origins.js#ORIGIN_MODULE`; birthday reminders are `event` rows but follow the `birthdays` switch, not `calendar`). Such rows are skipped, not deleted: task and calendar reminders are set by hand, subscription, inventory and document reminders only come into being when their record is saved, so none of them would come back. They stay pending and go out after the module is switched back on, like after a server outage (delivery has no age limit). Sources a periodic sync recreates - pantry, schedule, waste, cycle, birthdays - also clear their pending rows while the module is off and recreate them on the first run after it is back on
- **A module a member may not use sends them no reminders either (#1289):** the second permission axis, checked at the same place. `GET /reminders/pending` already filtered by `access_permissions` (`mayTouchOrigin()`); the push/channel delivery did not, so a member whose module access is `none` stopped seeing the toast but kept getting the push with its title, amount and date, and tapping it opened a page the route guard turns away. `processDueNotifications()` now resolves the recipient's permissions once per run (`server/services/reminder-origins.js#withoutModulesDeniedToRecipient`, same `ORIGIN_MODULE` map) and skips rows of a module denied to `reminders.created_by`. Only `none` counts, not `read`, and an admin is never filtered. No token scopes on this path: a scope belongs to a request, and a delivery run is not one. Skipped, not deleted, for the same reason as the household switch - a permission can be given back, and the row goes out on the next run after it is. Birthday reminders need no exception here: `birthdays` is a nav id under the `calendar` permission module, not a permission key of its own
...
- **Optional:** `MCP_INTERNAL_BASE_URL` overrides the base URL the bridge calls back into; it defaults to `BASE_URL`, otherwise to `http://<BIND_ADDRESS>:<PORT>` (`127.0.0.1` while `BIND_ADDRESS` is unset or `0.0.0.0`, `[::1]` for `::`), and is only needed when neither reaches the app.
...
- **Edge tokens in dark mode:** the three border steps are set independently under dark instead of being derived from the neutral ramp. The ramp sits so close to `--color-surface` there that `--color-border-subtle` resolved to **exactly the surface colour** (1.00:1) - a card edge in the colour of its own face, which left cards, list rows and form fields without a visible boundary. Dark therefore carries its own three values, each with its measured ratio next to it in `tokens.css`. **Form fields have their own edge token (#1230):** `--color-border-control` (`--neutral-500`) carries the resting edge of every input, select and textarea and holds the 3:1 that WCAG 1.4.11 asks of a control boundary on every ground a field sits on (light 3.18-3.53:1, dark 3.86-5.46:1). Until then fields used `--color-border` and rendered at 1.19-1.47:1. Card edges, separators and group outlines keep `--color-border`, so the stronger field edge does not harden every card in the app; the focus edge stays the accent.
